
# imp X3DH runs once

Send → DH ratchet

Receive → derive CKr

pendingDH controls DH rotation

All crypto uses raw bytes

Base64 only for storage/transport
# Curve25519 has two representations:

Representation	Used for	Name
Montgomery form	Diffie–Hellman	X25519
Twisted Edwards form	Signatures	Ed25519
# X25519 (Diffie–Hellman side)

Used for:
X3DH
Double Ratchet DH ratchet
Ephemeral keys
Prekeys
Operation:
shared = scalar × point

Properties:
Constant-time
Safe clamping
Simple API
In libsodium:
crypto_scalarmult()
crypto_kx_keypair()
# Ed25519 (Signature side)

Used for:
Identity keys
Signing signed-prekeys

Authenticating users/devices
Properties:
Very fast signatures
Deterministic signin
Strong security proofs

In libsodium:
crypto_sign_keypair()
crypto_sign_detached()
Curve25519
├── X25519 → Diffie–Hellman (shared secrets)
└── Ed25519 → Signatures (identity & auth)

 # Curve25519 has two representations:
Purpose	    Form
Signatures	Edwards (Ed25519)
DH	        Montgomery (X25519) 
  # imp:
IDENTITY KEY      → once in lifetime
SIGNED PREKEY     → rotate sometimes
ONE-TIME PREKEY   → use once
EPHEMERAL KEY     → per session
MESSAGE KEYS      → per message (from ratchet) 
# imp 2:
HOW LONG do keys live? (VERY IMPORTANT)
Key	Who	Lifetime 
Identity key (I)	    Alice/Bob	  🔒 Long-term (months/years)
Signed prekey (S) 	  Bob	        ⏱ Medium-term (days/weeks)
One-time prekey (O) 	Bob	        🔥 ONE TIME ONLY
Ephemeral key (E_A) 	Alice	      ❌ Deleted after X3DH
DH ratchet keys	      Both	     🔄 Rotated many times
Message keys (MK)	    Both	     🗑 Deleted immediately

  # 1 : Identity Key (IK) — generate ONCE, store forever
Generated once when user registers / installs app
Type: Ed25519 (signing key)
purpose:Proves “this is really Alice” Signs the Signed Prekey
Stored:Private key → encrypted on device
Public key → server (never changes)
✅ Never rotate
✅ Never per message
✅ Lifetime key
❓ Do we regenerate identity keys? ❌ No (unless user resets account)



# 2 : Signed Prekey (ROTATES, not lifetime)
What it is Medium-term key Generated after identity Signed using identity private key
Where it lives Private → on device .Public + signature → uploaded to server
How often Rotated every days / weeks NOT per message NOT lifetime
Why Allows offline users to receive messages Prevents MITM (because it’s signed by identity)
📌 Think of it as: temporary public contact number, verified by passport
❓ Do we regenerate signed prekeys? ✅ Yes (periodically)


# 3: OneTimePrekeys
A one-time prekey is a throw-away public key
that is used once, then deleted forever.
1: we have to stote privaet key at device an1 store public key at server
2: delete public after oen time use and fetch next from client
3:after use deletes that private key forever from both sides 



# 4 : Ephemeral Key (VERY SHORT-TERM) or session key 
Generated by the sender
Used in X3DH
Used for starting ONE session
Deleted after handshake
NOT used forever
NOT reused
👉 Think: “Handshake secret”
# WHEN EPHEMERAL KEY IS DELETED
On sender side:
Delete E_A private after X3DH
Never reuse it
Keep only derived session keys (RK, CK)

On Recevier side:
never stores E_A private (he doesn’t have it)
uses E_A public once
After session established → discard
X3DH (VERY IMPORTANT)

# X3DH,
# BOTH Alice and Bob already computed the SAME values:
RK  (Root Key)   ✅ same on both sides
CK  (Chain Key)  ✅ same on both sides
👉 They NEVER send RK or CK to each other
👉 They are the same because DH math guarantees it
this is where people get confused — so remember this:
🔒 Keys are NEVER sent. Only PUBLIC keys are sent.

# Double Ratchet
// has TWO ratchets:
🔁 Symmetric ratchet → per-message keys
🔄 DH ratchet → per-session reset
# Symmetric ratchet :CK into two chains
 # Alice side
CK_sending   = HKDF(CK, "send")
CK_receiving = HKDF(CK, "recv")

# Bob side (FLIPPED)
CK_receiving = HKDF(CK, "send")  ← same bytes as Alice send
CK_sending   = HKDF(CK, "recv")  ← same bytes as Alice recv

👉 This is how both sides stay in sync WITHOUT sending keys
# Alice side
CK_sending   = HKDF(CK, "send")
CK_receiving = HKDF(CK, "recv")

Bob side (FLIPPED)
CK_receiving = HKDF(CK, "send")  ← same bytes as Alice send
CK_sending   = HKDF(CK, "recv")  ← same bytes as Alice recv


👉 This is how both sides stay in sync WITHOUT sending keys
# Symmetric Ratchet
FIRST MESSAGE (Symmetric Ratchet ONLY)
Now Alice wants to send message #1.
 # Alice derives Message Key (MK₁)
MK1 = HKDF(CK_sending, "mk")
CK_sending = HKDF(CK_sending, "ck_next")
That’s it.
MK1 → encrypts message
CK_sending → moves forward
👉 Alice forgets MK1 forever after encrypting

4️⃣ Alice sends message to Bob
She sends ONLY this:
{
  ciphertext,
  nonce
}
❌ NO keys
❌ NO CK
❌ NO RK

5️⃣ Bob receives and decrypts (SAME STEPS)
Bob knows this is message #1 from Alice, so he uses his receiving chain:
MK1 = HKDF(CK_receiving, "mk")
CK_receiving = HKDF(CK_receiving, "ck_next")

👉 This MK1 is IDENTICAL to Alice’s MK1
Why?Because:
CK was same
HKDF inputs were same
Order is same
Now Bob decrypts successfully.

6️⃣ SECOND MESSAGE (still symmetric ratchet)

Alice sends message #2:

MK2 = HKDF(CK_sending, "mk")
CK_sending = HKDF(CK_sending, "ck_next")


Bob receives:

MK2 = HKDF(CK_receiving, "mk")
CK_receiving = HKDF(CK_receiving, "ck_next")


🔁 This continues for many messages.

7️⃣ IMPORTANT: Where is DH ratchet?

👉 NOT YET

DH ratchet happens ONLY when the OTHER SIDE replies
OR when someone sends a new DH public key in header

Right now:
Only symmetric ratchet is active
No DH key sent yet

8️⃣ VERY SIMPLE MENTAL MODEL (REMEMBER THIS)
After X3DH:
RK ──┐
     └─> CK ──> CK_send ──> MK1 ──> MK2 ──> MK3

Bob mirrors it:
CK_recv ──> MK1 ──> MK2 ──> MK3
NO COMMUNICATION about keys.

# Alice side
stateAlice = initDoubleRatchet(RK, CK, "alice");

msg = sendMessage(stateAlice, "hello bob");
→ send over server

 # Bob side
stateBob = initDoubleRatchet(RK, CK, "bob");

text = receiveMessage(stateBob, msg);
→ "hello bob"


✔ Same MK
✔ Same CK progression
✔ Server sees only ciphertext
✔ No keys shared directly
# Correct X3DH — Alice side (VERY IMPORTANT)
Alice has:

I_A → Alice identity key pair

E_A → Alice ephemeral key pair

Alice fetched from Bob:

I_B.publicKey

S_B.publicKey (signed prekey)

O_B.publicKey (one-time prekey, optional)
# Skipped message handling:
// state = {
  RK,
  CKs,
  CKr,

  DHs,
  DHr,

  Ns,
  Nr,

  skippedKeys: new Map() // 👈 REQUIRED # key = `${dhPubBase64}:${msgNumber}`
value = MK (Uint8Array)

};
//
# Now the 3 cases (THIS IS THE CORE)
✅ Case 1: header.n === state.Nr (NORMAL case)

Example:

header.n = 2
state.Nr = 2


Meaning:

“This is exactly the message I was expecting.”

What Bob does:

Derive one MK

Decrypt

Increment Nr

derive MK
decrypt
Nr++


✅ No skipping
✅ No storage
✅ Most common case

⏩ Case 2: header.n > state.Nr (OUT-OF-ORDER / FUTURE message)

Example:

header.n = 4
state.Nr = 2


Meaning:

“Alice jumped ahead — messages 2 and 3 are missing.”

What Bob must do:

Derive keys for messages 2 and 3

Store them as skipped keys

Then derive key for message 4

Visual:

Missing: 2, 3
Current:        4

Code logic:
skipMessageKeys(state, header.n);
derive MK for header.n
decrypt
Nr++


✅ Enables out-of-order delivery
✅ Prevents key loss
✅ REQUIRED by Signal

⏪ Case 3: header.n < state.Nr (LATE / DUPLICATE message)

Example:

header.n = 1
state.Nr = 3


Meaning:

“This message belongs to the past.”

Two possibilities:
a Legit late message

Bob previously stored the MK in MKSKIPPED

He decrypts using stored key

if (MKSKIPPED.has(keyId)) decrypt

b Replay / attack / duplicate

No key stored

Reject message

throw Error


❌ Never derive keys backwards
❌ Never reuse CK
# imp :
// app startup
const storageKey = await loadStorageKeyFromPassword(); // Argon2 / pwhash
let state = loadRatchetState(storageKey);

if (!state) {
  // only if NO previous session exists
  state = await initDoubleRatchetBob(RK, E_A_pub);
  persistRatchetState(state, storageKey);
}
# What “persist full ratchet state” really means

If the app:

crashes

reloads

device restarts

👉 you must be able to resume decryption and encryption
👉 without re-running X3DH
👉 without losing skipped keys

So we must persist ALL cryptographic state that affects future keys.
# Crash scenario (proof this works)
Scenario

Bob receives n=2

Skipped keys n=0,1 stored

App crashes

App restarts

Alice sends n=0

What happens

State loads

skippedKeys restored

MK(0) exists

Message decrypts

✅ No data loss
✅ No rekeying
✅ No broken chain